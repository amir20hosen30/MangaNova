...
async function load(q=''){let d=await api('/api/works?q='+encodeURIComponent(q));works=d.works;renderHome(works.slice(0,10));applyFilters()}
function card(w){return `<article class="card" onclick="location.href='/novel/${w.id}'"><div class="cover">${w.cover?`<img src="${w.cover}" alt="">`:''}<b>${w.title}</b></div><div class="info"><b>${w.title}</b><br><small>${w.kind} • ${w.chapter_count||0} فصل</small></div></article>`}
function renderHome(list){$('#homeWorks').innerHTML=list.map(card).join('')||'<p>هنوز اثری ثبت نشده است.</p>'}
