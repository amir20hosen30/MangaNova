...
async function init(){let m=await api('/api/me');me=m.user;applyUserSettings();renderHeaderProfile();await load();renderProfile();routeFromUrl();}
...
function applyUserSettings(){
  const s=me?.settings||{};
  const colors={orange:'#ff6b35',purple:'#9a4dff',blue:'#3b82f6',green:'#22c55e'};
  document.documentElement.style.setProperty('--a',colors[s.accent]||colors.orange);
  document.documentElement.style.setProperty('--a2',colors[s.accent]||colors.orange);
  document.body.dataset.accent=s.accent||'orange';
}
function settingsModal(){
  if(!me)return auth();
  const s={accent:'orange',notifications:true,history:true,readerWidth:100,...(me.settings||{})};
  openModal(`<div class="box settings-box"><button class="close" onclick="closeModal()">×</button><div class="settings-head"><div class="notice-icon">${icon('settings')}</div><div><h2>تنظیمات</h2><p class="muted">ظاهر و تجربه خواندن MangaNova را شخصی‌سازی کن.</p></div></div>
  <div class="settings-section"><h3>ظاهر سایت</h3><label class="setting-label">رنگ تأکیدی</label><div class="accent-options">${[['orange','نارنجی'],['purple','بنفش'],['blue','آبی'],['green','سبز']].map(([v,l])=>`<button class="accent-choice ${s.accent===v?'selected':''}" data-accent="${v}" type="button"><i></i>${l}</button>`).join('')}</div></div>
  <div class="settings-section"><h3>اعلان‌ها</h3><label class="switch-row"><span><b>اعلان فصل‌های جدید</b><small>وقتی قابلیت اعلان فعال باشد، اطلاع‌رسانی‌های مربوط به آثار دنبال‌شده را دریافت می‌کنی.</small></span><input id="setNotifications" type="checkbox" ${s.notifications?'checked':''}><span class="switch"></span></label></div>
  <div class="settings-section"><h3>مطالعه</h3><label class="switch-row"><span><b>ذخیره تاریخچه مطالعه</b><small>آخرین فصل و صفحه‌ای که خوانده‌ای ذخیره شود.</small></span><input id="setHistory" type="checkbox" ${s.history?'checked':''}><span class="switch"></span></label><label class="setting-label">عرض تصاویر در مطالعه</label><div class="width-options">${[80,100,120].map(v=>`<button class="width-choice ${s.readerWidth===v?'selected':''}" data-width="${v}" type="button">${v}%</button>`).join('')}</div></div>
  <div class="settings-actions"><button class="ghost" onclick="closeModal()">انصراف</button><button class="primary" id="saveSettings">ذخیره تنظیمات</button></div></div>`);
  let accent=s.accent,width=s.readerWidth;
  document.querySelectorAll('.accent-choice').forEach(b=>b.onclick=()=>{accent=b.dataset.accent;document.querySelectorAll('.accent-choice').forEach(x=>x.classList.toggle('selected',x===b));document.documentElement.style.setProperty('--a',({orange:'#ff6b35',purple:'#9a4dff',blue:'#3b82f6',green:'#22c55e'})[accent]);});
  document.querySelectorAll('.width-choice').forEach(b=>b.onclick=()=>{width=+b.dataset.width;document.querySelectorAll('.width-choice').forEach(x=>x.classList.toggle('selected',x===b));});
  $('#saveSettings').onclick=async()=>{try{const d=await api('/api/profile/settings',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({accent,notifications:$('#setNotifications').checked,history:$('#setHistory').checked,readerWidth:width})});me.settings=d.settings;applyUserSettings();closeModal();profileNotice('تنظیمات ذخیره شد','تغییرات حساب کاربری و تنظیمات مطالعه با موفقیت ذخیره شد.');}catch(e){alert(e.message)}};
}
...
<div class="profile-menu">
 <button class="profile-menu-item" onclick="settingsModal()"><span class="menu-icon">${icon('settings')}</span><span>تنظیمات</span><span class="menu-arrow">‹</span></button>
...
async function track(w,c,p){if(me&&me.settings?.history!==false&&p%3===0)api('/api/history',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({work_id:w,chapter_id:c,last_page:p})}).catch(()=>{})}
