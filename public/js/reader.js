...
Promise.all([api('/api/chapters/'+cid()),api('/api/me').catch(()=>({user:null}))]).then(([c,m])=>{chapter=c;const w=m.user?.settings?.readerWidth;if([80,100,120].includes(+w))zoom=+w/100;render()}).catch(e=>$('#readerPages').innerHTML='<div class="reader-loading">'+e.message+'</div>');

// Discourage saving/copying reader pages from the normal browser UI. This cannot prevent screenshots.
document.addEventListener('contextmenu',e=>e.preventDefault());
document.addEventListener('dragstart',e=>{if(e.target.closest?.('.manga-page'))e.preventDefault()});
document.addEventListener('selectstart',e=>{if(e.target.closest?.('.manga-page'))e.preventDefault()});
document.addEventListener('keydown',e=>{
