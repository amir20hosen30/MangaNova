...
function ticketOut(t){return {id:t.id,user_id:t.user_id,username:t.username,subject:t.subject,status:t.status,admin_seen:t.admin_seen,user_unread:t.user_unread,created_at:t.created_at,updated_at:t.updated_at,messages:t.messages}}
...
app.post('/api/me/tickets',auth,(req,res)=>{const subject=String(req.body?.subject||'').trim().slice(0,120),message=String(req.body?.message||'').trim().slice(0,2000);if(!subject||!message)return res.status(400).json({error:'موضوع و متن پیام را وارد کنید.'});const now=new Date().toISOString();const t={id:next('tickets'),user_id:user(req).id,username:user(req).username,subject,status:'open',admin_seen:false,user_unread:false,messages:[{from:'user',text:message,at:now}],created_at:now,updated_at:now};db.tickets.push(t);save();res.json({ticket:ticketOut(t)})});
app.post('/api/me/tickets/:id/reply',auth,(req,res)=>{const t=db.tickets.find(x=>x.id===+req.params.id&&x.user_id===user(req).id);if(!t)return res.status(404).json({error:'تیکت پیدا نشد'});const text=String(req.body?.message||'').trim().slice(0,2000);if(!text)return res.status(400).json({error:'متن پیام را وارد کنید.'});const now=new Date().toISOString();t.messages.push({from:'user',text,at:now});if(t.status!=='open')t.status='open';t.admin_seen=false;t.updated_at=now;save();res.json({ticket:ticketOut(t)})});
...
app.post('/api/admin/tickets/:id/reply',admin,(req,res)=>{const t=db.tickets.find(x=>x.id===+req.params.id);if(!t)return res.status(404).json({error:'تیکت پیدا نشد'});const text=String(req.body?.message||'').trim().slice(0,3000);if(!text)return res.status(400).json({error:'متن پاسخ را وارد کنید.'});const now=new Date().toISOString();t.messages.push({from:'admin',text,at:now});const status=req.body?.status;t.status=['open','answered','closed'].includes(status)?status:'answered';t.admin_seen=true;t.user_unread=true;t.updated_at=now;save();res.json({ticket:ticketOut(t)})});
