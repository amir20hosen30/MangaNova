...
// Public assets exclude chapter pages so users cannot download them directly.
app.use('/uploads/covers',express.static(path.join(UPLOADS_DIR,'covers'),{maxAge:'7d'}));
app.use('/uploads/avatars',express.static(path.join(UPLOADS_DIR,'avatars'),{maxAge:'7d'}));
...
app.get('/api/me',(req,res)=>{const sess=user(req);if(!sess)return res.json({user:null});const u=db.users.find(x=>x.id===sess.id);if(u){sess.avatar=u.avatar||'';sess.settings={accent:'orange',notifications:true,history:true,readerWidth:100,...(u.settings||{})}}res.json({user:sess})});
app.get('/api/profile/settings',auth,(req,res)=>{const u=db.users.find(x=>x.id===user(req).id);if(!u)return res.status(404).json({error:'کاربر پیدا نشد'});u.settings={accent:'orange',notifications:true,history:true,readerWidth:100,...(u.settings||{})};res.json({settings:u.settings})});
app.patch('/api/profile/settings',auth,(req,res)=>{const u=db.users.find(x=>x.id===user(req).id);if(!u)return res.status(404).json({error:'کاربر پیدا نشد'});const body=req.body||{},allowedAccent=['orange','purple','blue','green'],allowedWidth=[80,100,120];const current={accent:'orange',notifications:true,history:true,readerWidth:100,...(u.settings||{})};if(allowedAccent.includes(body.accent))current.accent=body.accent;if(typeof body.notifications==='boolean')current.notifications=body.notifications;if(typeof body.history==='boolean')current.history=body.history;if(allowedWidth.includes(Number(body.readerWidth)))current.readerWidth=Number(body.readerWidth);u.settings=current;req.session.user.settings=current;save();res.json({settings:current})});
app.post('/api/register',(req,res)=>{const {username,password}=req.body||{};if(!username||!password||password.length<6)return res.status(400).json({error:'نام کاربری و رمز عبور معتبر لازم است (رمز حداقل ۶ کاراکتر).'});if(db.users.some(u=>u.username===username))return res.status(409).json({error:'این نام کاربری قبلاً ثبت شده است.'});const u={id:next('users'),username,password:bcrypt.hashSync(password,10),role:'user',avatar:'',settings:{accent:'orange',notifications:true,history:true,readerWidth:100},created_at:new Date().toISOString()};db.users.push(u);save();req.session.user={id:u.id,username:u.username,role:u.role,avatar:''};res.json({user:req.session.user})});
...
app.get('/api/chapters/:id',auth,(req,res)=>{const c=db.chapters.find(x=>x.id===+req.params.id);if(!c)return res.status(404).json({error:'فصل پیدا نشد'});const w=db.works.find(x=>x.id===c.work_id);const pages=db.pages.filter(p=>p.chapter_id===c.id).sort((a,b)=>a.page_no-b.page_no).map(p=>({...p,image:'/api/pages/'+p.id+'/image'}));const same=db.chapters.filter(x=>x.work_id===c.work_id).sort((a,b)=>a.number-b.number);const idx=same.findIndex(x=>x.id===c.id);res.json({...c,work_title:w?.title||'',pages,prev:same[idx-1]||null,next:same[idx+1]||null})});

// Chapter pages are protected and are never exposed through the public /uploads path.
app.get('/api/pages/:id/image',auth,(req,res)=>{
  const p=db.pages.find(x=>x.id===+req.params.id);
  if(!p)return res.status(404).end();
  const c=db.chapters.find(x=>x.id===p.chapter_id);
  if(!c)return res.status(404).end();
  const rel=String(p.image||'').replace(/^\/uploads\/chapters\//,'');
  if(!rel || rel.includes('..'))return res.status(404).end();
  const file=path.resolve(path.join(UPLOADS_DIR,'chapters',rel));
  const root=path.resolve(path.join(UPLOADS_DIR,'chapters'));
  if(!file.startsWith(root+path.sep)||!fs.existsSync(file))return res.status(404).end();
  res.set({'Cache-Control':'no-store, no-cache, must-revalidate, private','Pragma':'no-cache','X-Content-Type-Options':'nosniff','Content-Disposition':'inline'});
  res.sendFile(file);
});
